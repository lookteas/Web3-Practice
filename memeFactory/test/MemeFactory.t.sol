...
contract MemeFactoryTest is Test {
    ...
    function testDeployInscription() public {
        ...
        emit TokenDeployed(address(0), "TEST", 1000000 * 10**18, 1000 * 10**18, user1);
        
        address tokenAddr = factory.deployInscription{value: DEPLOYMENT_FEE}(
            "TEST",
    ...
    function testMintInscription() public {
        ...
        emit TokenMinted(tokenAddr, user2, 1000 * 10**18, user2);
        
        factory.mintInscription{value: MINTING_FEE}(tokenAddr);
    ...
    function testDeployInscriptionInvalidParams() public {
        vm.startPrank(user1);
        
        // 测试空符号
        vm.expectRevert("Symbol cannot be empty");
        factory.deployInscription{value: DEPLOYMENT_FEE}("", 1000, 100);
        
        // 测试符号过长
        vm.expectRevert("Symbol too long");
        factory.deployInscription{value: DEPLOYMENT_FEE}("VERYLONGSYMBOL", 1000, 100);
        
        // 测试总供应量为0
        vm.expectRevert("Total supply must be greater than 0");
        factory.deployInscription{value: DEPLOYMENT_FEE}("TEST", 0, 100);
        
        // 测试每次铸造量为0
        vm.expectRevert("Per mint must be greater than 0");
        factory.deployInscription{value: DEPLOYMENT_FEE}("TEST", 1000, 0);
        
        // 测试每次铸造量大于总供应量
        vm.expectRevert("Per mint cannot exceed total supply");
        factory.deployInscription{value: DEPLOYMENT_FEE}("TEST", 100, 1000);
        
        // 测试费用不足
        vm.expectRevert("Insufficient deployment fee");
        factory.deployInscription{value: DEPLOYMENT_FEE - 1}("TEST", 1000, 100);
    ...
    function testDuplicateSymbol() public {
        // 部署第一个代币
        vm.prank(user1);
        factory.deployInscription{value: DEPLOYMENT_FEE}("DUP", 1000, 100);
        
        // 尝试部署相同符号的代币
        vm.prank(user2);
        vm.expectRevert("Symbol already exists");
        factory.deployInscription{value: DEPLOYMENT_FEE}("DUP", 2000, 200);
    }
    
    function testMintInscriptionInvalidToken() public {
        vm.startPrank(user1);
        
        // 测试无效地址
        vm.expectRevert("Invalid token address");
        factory.mintInscription{value: MINTING_FEE}(address(0));
        
        // 测试非工厂部署的代币
        vm.expectRevert("Token not deployed by this factory");
        factory.mintInscription{value: MINTING_FEE}(address(0x123));
        
        // 测试费用不足
        address tokenAddr = factory.deployInscription{value: DEPLOYMENT_FEE}("FEE", 1000, 100);
        vm.expectRevert("Insufficient minting fee");
        factory.mintInscription{value: MINTING_FEE - 1}(tokenAddr);
    ...
    function testBatchMintInscription() public {
        // 部署代币
        vm.prank(user1);
        address tokenAddr = factory.deployInscription{value: DEPLOYMENT_FEE}(
            "BATCH",
        ...
        // 批量铸造
        vm.prank(user2);
        factory.batchMintInscription{value: MINTING_FEE * 3}(tokenAddr, 3);
    ...
    function testBatchMintInvalidCount() public {
        vm.prank(user1);
        address tokenAddr = factory.deployInscription{value: DEPLOYMENT_FEE}("COUNT", 1000, 100);
        
        vm.startPrank(user2);
        
        // 测试数量为0
        vm.expectRevert("Invalid count (1-5)");
        factory.batchMintInscription{value: MINTING_FEE}(tokenAddr, 0);
        
        // 测试数量超过限制
        vm.expectRevert("Invalid count (1-5)");
        factory.batchMintInscription{value: MINTING_FEE * 6}(tokenAddr, 6);
        
        // 测试费用不足
        vm.expectRevert("Insufficient minting fee");
        factory.batchMintInscription{value: MINTING_FEE * 2}(tokenAddr, 3);
    ...
    function testGetDeployedTokens() public {
        // 部署多个代币
        vm.startPrank(user1);
        address token1 = factory.deployInscription{value: DEPLOYMENT_FEE}("TOK1", 1000, 100);
        address token2 = factory.deployInscription{value: DEPLOYMENT_FEE}("TOK2", 2000, 200);
        address token3 = factory.deployInscription{value: DEPLOYMENT_FEE}("TOK3", 3000, 300);
        vm.stopPrank();
    ...
    function testGetTokenBySymbol() public {
        vm.prank(user1);
        address tokenAddr = factory.deployInscription{value: DEPLOYMENT_FEE}("SYM", 1000, 100);
    ...
    function testIsSymbolAvailable() public {
        ...
        vm.prank(user1);
        factory.deployInscription{value: DEPLOYMENT_FEE}("TAKEN", 1000, 100);
    ...
    function testWithdraw() public {
        // 先收集一些费用
        vm.prank(user1);
        factory.deployInscription{value: DEPLOYMENT_FEE}("WITHDRAW", 1000, 100);
    ...
    function testGasUsage() public {
        ...
        factory.deployInscription{value: DEPLOYMENT_FEE}("GAS", 1000000 * 10**18, 1000 * 10**18);
        uint256 gasUsed = gasBefore - gasleft();
        
        // 记录 gas 使用情况（用于优化参考）
        console.log("Deploy gas used:", gasUsed);
